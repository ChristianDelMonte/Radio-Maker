VERSION 5.00
Begin VB.UserControl rmOptionButton 
   AutoRedraw      =   -1  'True
   ClientHeight    =   3600
   ClientLeft      =   0
   ClientTop       =   0
   ClientWidth     =   4800
   PropertyPages   =   "rmOption.ctx":0000
   ScaleHeight     =   3600
   ScaleWidth      =   4800
   ToolboxBitmap   =   "rmOption.ctx":0040
   Begin VB.PictureBox Image1 
      AutoSize        =   -1  'True
      BorderStyle     =   0  'None
      Height          =   195
      Left            =   600
      Picture         =   "rmOption.ctx":0352
      ScaleHeight     =   195
      ScaleWidth      =   1170
      TabIndex        =   0
      Top             =   960
   ...
   End
End
Attribute VB_Name = "rmOptionButton"
...
'=======================================================
'déchecke tous les rmOption de la form ayant la même valeur de Group
'=======================================================
Private Sub UnCheckOther()
Dim Obj As Control

    For Each Obj In Parent.Controls
        If TypeOf Obj Is rmOptionButton Then
...
Dim x As Long
    ...
    'd'un rmOption sur le controle container, et prend son group
    
    'récupère tous les Group de tous les controles rmOption de la form
    x = 0
    For Each Obj In Parent.Controls
        If TypeOf Obj Is rmOptionButton Then
    ...
    If x = 0 Then
        'alors il n'y avait pas de rmOption dans cette form
        Me.Group = 0
        bNotOk2 = False
    ...
    'récupère le handle du container de CE rmOption
    lngContainerHandle = UserControl.ContainerHwnd
    
    'on recherche maintenant tous les rmOption (sauf CE rmOption) pour voir si
    'ils ont le même controle container
    For Each Obj In Parent.Controls
        If TypeOf Obj Is rmOptionButton And Obj.Name <> Ambient.DisplayName Then
    ...
    'si on est là, on n'a trouvé aucun rmOption ayant le même controle container
    'mais il existe déjà des rmOption, donc on me donne un Group non existant
    For x = 0 To 255
        If lGroup(x) = 0 Then
