...
Public Sub RipMP3(outMP3Name As String, StartAddr As Long, EndAddr As Long, beConfig As PBE_CONFIG)
  ...
  Do While LLen
    ...
    frmProgress.ChangeProgress "Extrayendo pista de audio " & outMP3Name, CSng((temp - LLen)), CSng(temp)
    DoEvents
  Loop
...
Public Function RipWAV(Filename As String, addrStart As Long, addrEnd As Long)
  ...
  Do While LLen
    ...
    'Inform user where we go...
    frmProgress.ChangeProgress "Extrayendo pista de audio " & Filename, CSng(temp - LLen), CSng(temp)
    DoEvents
  Loop
